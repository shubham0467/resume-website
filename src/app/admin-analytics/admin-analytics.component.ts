...
export class AdminAnalyticsComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<any>(`${environment.API_URL}/admin-analytics?password=admin123`).subscribe(data => {
  ...
  private pageNameMap: Record<string, string> = {
    ...
    '/contact': 'Contact', '/analytics': 'Analytics',
    '/chatbot': 'Chatbot', '/admin-login': 'Admin Login'
  };
  ...
  getPageLabel(raw: string) {
    return this.pageNameMap[raw] ?? raw;
  }
}
